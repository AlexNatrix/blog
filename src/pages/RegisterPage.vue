  ...
  <div class="body-content">
    <div class="module">
      ...
      <form class="form" autocomplete="off">
        ...
        <input v-model="passwd" placeholder="Confirm Password" required />
        <button
          @click="validate"
          type="button"
...
export default {
  ...
  methods: {
    async register() {
      let k = "";
      try {
        const str = { email: this.User.email, password: this.User.password };
        console.log(str);
        ...
        if (k !== `Email ${this.User.email} already registered!`) {
          window.location = "/completeyourregistration";
        }
      }
    },
    validate() {
      console.log(this.passwd === this.User.password);
      this.register();
    },
  },
};
...
body {
  ...
  background: url("https://miro.medium.com/max/1400/0*GMReHtD9mDbpl54Q") #0f2439
...
input {
  ...
  font-size: 15px;
  line-height: 20px;
  border: 1px solid rgba(7, 123, 190, 0.3);
  background-color: #6ab3c97e;
...
input {
  ...
  background-color: rgba(47, 140, 194, 0.3);
  color: rgba(17, 16, 16, 0.7);
}
select:focus,
textarea:focus,
input:focus {
  border: 2px solid;
  border-color: #1e5f99;
  background-color: rgba(7, 123, 190, 0.1);
  color: rgba(17, 16, 16, 0.7);
}
.btn {
