<template>
  ...
  <h1>Posts</h1>
  <my-input v-focus ref="search" v-model="searchQuery" placeholder="Search..."></my-input>
  ...
  <my-dialog v-model:show="dialogVisible" @updateShow="hideDialog">
    ...
    <post-list :posts="sortedAndSearchedPosts" @deletePost = "deletePost" v-if="!isPostLoading"/>
...
components: { PostList, PostForm, MyButton },
  ...
  setup(){
    ...
    const {sortedAndSearchedPosts,searchQuery}  = userSortedAndSearchedPosts(sortedPosts);
    return{
        posts,
        totalPages,
        isPostLoading,
